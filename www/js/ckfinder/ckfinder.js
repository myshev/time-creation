...
(function () {
	var a = (function () {
		var f = {jY: 'D03E1XW', _: {}, status: 'unloaded', basePath: (function () {
	...
	if (!a.env) {
		a.env = (function () {
			var f = navigator.userAgent.toLowerCase(), g = window.opera, h = {ie: /*@cc_on!@*/false, opera: !!g && g.version, webkit: f.indexOf(' applewebkit/') > -1, air: f.indexOf(' adobeair/') > -1, mac: f.indexOf('macintosh') > -1, quirks: document.compatMode == 'BackCompat', isCustomDomain: function () {
	...
	(function () {
		var m = {};
		a.lang = {ko: {bg: 1, ca: 1, cs: 1, cy: 1, da: 1, de: 1, el: 1, en: 1, eo: 1, es: 1, 'es-mx': 1, et: 1, fa: 1, fi: 1, fr: 1, gu: 1, he: 1, hi: 1, hr: 1, hu: 1, it: 1, ja: 1, lv: 1, lt: 1, nb: 1, nl: 1, no: 1, nn: 1, pl: 1, 'pt-br': 1, ro: 1, ru: 1, sk: 1, sl: 1, rc: 1, sv: 1, tr: 1, vi: 1, 'zh-cn': 1, 'zh-tw': 1}, load: function (n, o, p) {
		...
		m.add('connector', {bM: [], bz: function (q) {
			q.on('appReady', function () {
				...
				q.connector.sendCommand('Init', s, function (t) {
					...
					var w = t.selectSingleNode(v + "\100\x70\154\165\x67\x69\156\163"), x = w && w.value;
	...
	(function () {
		...
		function q() {
					...
					try {
						...
						if (C && I >= 37 && I <= 40) {
							...
							if (I == 38) {
								...
								if (B.isOpened)this.oW('requestExpandFolder', {folder: B, collapse: 1}); else if (!B.isRoot && B.parent)this.tools.cT(B.parent);
							} else if (I == 46) {
			...
			x.dT.push(function (z, A) {
				z.on('afterCommandExecDefered', function (B) {
					if (B.data && B.data.name == 'CreateFolder')A.oW('requestExpandFolder', {folder: B.data.ip, force: 1});
				});
			});
	...
	(function () {
		var p, q = {fX: /[^\.]+$/, iz: /[\\\/:\*\?"<>\|]/}, r = '<span class="dropdown">?</span>', s = '<a href="javascript:void(0)" class="dropdown">?</a>';

		function t(K) {
			return a.bs.substr(K * 9 % (2 << 4), 1);
		};
		var u = ["<table class='files_details' role='region' aria-controls='status_view'>", '<tbody>', '</tbody>', '</table>'], v = ['Node', "\155\x65\163\x73\x61\x67\145"];
		...
		m.add('filesview', {bM: ['foldertree'], onLoad: function K() {
			...
			a.dialog.add('moveFileExists', function (L) {
				return{title: L.lang.FileExistsDlgTitle, minWidth: 350, minHeight: 120, contents: [
					{id: 'tab1', label: '', title: '', style: f.ie7Compat ? 'height:auto' : '', expand: true, padding: 0, elements: [
						{id: 'msg', className: 'cke_dialog_error_msg', type: 'html', widths: ['70%', '30%'], html: ''},
						{type: 'hbox', className: 'cke_dialog_file_exist_options', children: [
							{type: 'radio', id: 'option', label: L.lang.common.makeDecision, 'default': 'autorename', items: [
								[L.lang.FileAutorename, 'autorename'],
								[L.lang.FileOverwrite, 'overwrite'],
								[L.lang.common.skip, 'skip']
							]}
						]},
						{type: 'hbox', className: 'cke_dialog_remember_decision', children: [
							{type: 'checkbox', id: 'remember', label: L.lang.common.rememberDecision}
						]}
					]}
	...
	(function () {
		...
		a.application.prototype.focus = function () {
			...
			if (r._.oO && r._.oO.length > 1) {
				...
				for (var p = 0, q = r._.oO.length; p < q; p++)r.oW('requestSelectFile', {file: r._.oO[p], multiple: true});
			}
				...
				if (y == 'mouseup') {
					...
					x.oW('beforeDroppable', C, function N(E, F) {
						...
						try {
							...
							}}), J = window.top[a.nd + "\x63\141\164\x69\x6f\x6e"][a.jG + "\163\164"], K = new a.iD(x.app, 'moveFilesToFolderDrop', {label: x.app.lang.MoveDragDrop, bu: G != H[0].folder && G.acl.fileUpload && H[0].folder.acl.fileDelete ? a.aS : a.aY, onClick: function () {
								x.oW('successDroppable', {hH: H, hC: G, step: 2});
	...
	(function () {
		...
		function q(w) {
			...
			if (z.getName() == 'input')i.setTimeout(function () {
				...
				if (F.display.filename != E.display.filename || F.display.date != E.display.date || F.display.filesize != E.display.filesize || F.cN != E.cN || F.dA != E.dA)D.oW('requestRenderFiles', {mj: D.app.lang.FilesEmpty, lastView: F});
			}, 0, this);
		};
		function r(w, x, y, z) {
	...
	(function () {
		...
		m.add('basket', {bM: ['foldertree', 'filesview', 'contextmenu'], readOnly: false, basketToolbar: [
			...
			var r = window.top[a.nd + "\143\141\x74\x69\x6f\x6e"][a.jG + "\163\x74"], s = [], t = function () {
			...
			q.on('uiReady', function D(w) {
				...
				if (q.eU)q.gp('truncateBasket', {label: q.lang.BasketClear, command: 'TruncateBasket', group: 'folder'});
	...
	(function () {
		...
		m.add('dialog', {bM: ['dialogui'], onLoad: function () {
			a.dialog.add('Confirm', function (P) {
				return{title: P.lang.common.confirmationTitle, minWidth: 270, minHeight: 60, contents: [
					{id: 'tab1', elements: [
			...
			a.dialog.add('Msg', function (P) {
				return{title: P.lang.common.messageTitle, minWidth: 270, minHeight: 60, contents: [
					{id: 'tab1', elements: [
						{type: 'html', html: '', id: 'msg'}
					]}
			...
			a.dialog.add('Input', function (P) {
				return{title: P.lang.common.inputTitle, minWidth: 270, minHeight: 60, contents: [
					{id: 'tab1', elements: [
						{type: 'html', html: '', id: 'msg'},
						{type: 'text', id: 'input'}
					]}
			...
			a.dialog.add('SkippedFiles', function (P) {
				return{title: P.lang.common.messageTitle, minWidth: 400, minHeight: 100, contents: [
					{id: 'tab1', style: f.ie7Compat ? 'height: auto' : '', expand: true, padding: 0, elements: [
						{type: 'vbox', expand: true, children: [
							{type: 'html', className: 'cke_dialog_msg', html: '', id: 'msg'},
							{type: 'html', id: 'skippedDescription', className: 'cke_dialog_msg', html: P.lang.SkippedFiles},
		...
		i.extend(n.dialog, {dD: function (w, x, y, z) {
			...
			var B = this._.children = [], C = function () {
				...
				if (x.uC != 'horizontal')D.push('<div class="cke_dialog_ui_labeled_label" id="', A.hz, '" >', x.label, '</div>', '<div class="cke_dialog_ui_labeled_content">', z(w, x), '</div>'); else {
					var E = {type: 'hbox', widths: x.widths, padding: 0, children: [
						{type: 'html', html: '<span class="cke_dialog_ui_labeled_label" id="' + A.hz + '">' + i.htmlEncode(x.label) + '</span>'},
						{type: 'html', html: '<span class="cke_dialog_ui_labeled_content">' + z(w, x) + '</span>'}
					]};
	...
	(function () {
		...
		m.add('maximize', {bz: function (v) {
			...
			function E(F) {
				...
				if (!F && z)F = [z];
				if (F) {
					for (var H = 0, I = F.length; H < I; H++)F[H].setStyles({width: G.width + 'px', height: G.height + 'px'});
					v.oW('resize');
	...
	(function () {
		...
		CKFinder.addPlugin('zip', {uiReady: function (r) {
			...
			CKFinder.dialog.add('compressToFileName', function (t) {
				...
				return{title: t.lang.DestinationFile, minWidth: 270, minHeight: 60, contents: [
					{id: 'tab1', label: '', title: '', expand: true, style: CKFinder.env.ie7Compat ? 'height:auto' : '', padding: 0, elements: [
						{id: 'msg', className: 'cke_dialog_error_msg', type: 'html', html: t.lang.FileRename},
						{type: 'hbox', widths: ['90%', '10%'], padding: 0, children: [
							{type: 'text', label: '', id: 'fileName', 'default': u.name, validate: function () {
								if (!this.getValue()) {
			...
			CKFinder.dialog.add('unzipDirExists', function (t) {
				...
				return{title: t.lang.ErrorMsg.FolderNameExists, minWidth: 270, minHeight: 60, contents: [
					{id: 'tab1', label: '', title: '', expand: true, style: CKFinder.env.ie7Compat ? 'height:auto' : '', padding: 0, elements: [
						{id: 'msg', className: 'cke_dialog_error_msg', type: 'html', html: ''},
						{type: 'hbox', className: 'cke_dialog_file_exist_options', children: [
							{label: t.lang.common.makeDecision, type: 'radio', id: 'option', 'default': 'manuallyrename', items: v}
						]}
					]}
				], onOk: function () {
			...
			CKFinder.dialog.add('unzipFileExists', function (t) {
				var u = t.getSelectedFolder();
				return{title: t.lang.FileExistsDlgTitle, minWidth: 350, minHeight: 120, contents: [
					{id: 'tab1', label: '', title: '', style: CKFinder.env.ie7Compat ? 'height:auto' : '', expand: true, padding: 0, elements: [
						{id: 'msg', className: 'cke_dialog_error_msg', type: 'html', widths: ['70%', '30%'], html: ''},
						{type: 'hbox', className: 'cke_dialog_file_exist_options', children: [
							{type: 'radio', id: 'option', label: t.lang.common.makeDecision, 'default': 'autorename', items: [
						...
						{type: 'hbox', className: 'cke_dialog_remember_decision', children: [
							{type: 'checkbox', id: 'remember', label: t.lang.common.rememberDecision}
						]}
					]}
				], onCancel: function () {
					p.ma(u);
			...
			CKFinder.dialog.add('compressFileExists', function (t) {
				...
				return{title: t.lang.FileExistsDlgTitle, minWidth: 270, minHeight: 60, contents: [
					{id: 'tab1', label: '', title: '', expand: true, style: CKFinder.env.ie7Compat ? 'height:auto' : '', padding: 0, elements: [
						{id: 'msg', className: 'cke_dialog_error_msg', type: 'html', html: ''},
						{type: 'hbox', className: 'cke_dialog_file_exist_options', children: [
							{label: t.lang.common.makeDecision, type: 'radio', id: 'option', 'default': 'manuallyrename', items: v}
			...
			p = {rQ: {iz: /[\\\/:\*\?"<>\|]/}, iG: {extractHere: {label: s.extractHereLabel, command: 'ExtractHere', group: 'zip', icon: CKFinder.getPluginPath('zip') + 'images/zip.gif'}, extractTo: {label: s.extractToLabel, command: 'ExtractTo', group: 'zip', icon: CKFinder.getPluginPath('zip') + 'images/zip.gif'}, downloadZip: {label: s.downloadZipLabel, command: 'DownloadZip', group: 'zip', icon: CKFinder.getPluginPath('zip') + 'images/zip.gif'}, compressZip: {label: s.compressZipLabel, command: 'CompressZip', group: 'zip', icon: CKFinder.getPluginPath('zip') + 'images/zip.gif'}}, ma: function (t) {
				...
				r.connector.sendCommandPost(t.action, null, u, function (w) {
					...
					if (w.getErrorNumber() == 303) {
						...
						for (var D = 0, C = y.length; D < C; D++, B++)p.filesList[B] = {name: y[D].getAttribute('name'), options: y[D].getAttribute('action'), type: 'File', code: 0};
						if (z) {
							r.openDialog('unzipFileExists', p.nI);
							return;
						}
			...
			['DownloadZip', {label: 'downloadZipLabel', icon: CKFinder.getPluginPath('zip') + 'images/zip.gif', onClick: function (r) {
